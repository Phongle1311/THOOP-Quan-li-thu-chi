...
void Loan::EnterInterRateDebtD() {
	if (interestRateD.size() < durationD) {
		...
		if (interestRateD.size() > 0)
			interestRateD.pop_back();
...
double Loan::GetDebtS() {
	return debtS;
}

double Loan::GetDebtD() {
	return debtD;
}

double Loan::GetRateS() {
	return interestRateS;
}

vector<double> Loan::GetRatesD() {
	return interestRateD;
}

double Loan::GetRateD() {
	return interestRateD.back();
}
